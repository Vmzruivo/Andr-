...
export async function signUp(email, password, name) {
  ...
  const { data, error } = await supabase.auth.signUp({
    email: cleanEmail,
    password,
    options: { data: { name: cleanName }, emailRedirectTo: redirectTo },
  });
  if (error) {
    const msg = String(error.message || "");
    if (/email rate limit exceeded|rate limit|after\s+60\s+seconds|60\s+seconds/i.test(msg)) {
      throw new Error("O Supabase bloqueou temporariamente o envio de e-mails por limite de segurança. Aguarde o bloqueio terminar antes de tentar novamente. Não clique várias vezes em Registrar.");
    }
    if (/already registered|already been registered|user already registered/i.test(msg)) {
      throw new Error("Este e-mail já possui uma conta. Use Entrar em vez de Registrar.");
    }
    throw error;
  }
...
export async function signIn(email, password) {
  ...
  if (error) {
    if (/email not confirmed/i.test(error.message || "")) {
      throw new Error("Sua conta ainda não foi confirmada. Abra o e-mail de confirmação e depois tente entrar novamente.");
    }
    throw error;
  }
  return data;
...
export function subscribeToFeed(onChange) { return supabase.channel("codex-feed").on("postgres_changes", { event: "*", schema: "public", table: "posts" }, onChange).on("postgres_changes", { event: "*", schema: "public", table: "likes" }, onChange).subscribe(); }
