...
export async function getFeed(limit = 30) {
  const { data, error } = await supabase
    .from("posts")
    .select("*, profiles!posts_author_id_fkey(id,name,avatar_url,level,total_xp,quests_completed_ever,max_streak_ever,usage_seconds)")
...
export async function getLeaderboard(mode = "level", limit = 50) {
  ...
  const { data, error } = await supabase
    .from("profiles")
    .select("id,name,avatar_url,level,total_xp,usage_seconds,quests_completed_ever,max_streak_ever")
...
export async function createPrivateConversation(myId, otherId) {
  ...
  const { error: firstMemberError } = await supabase
    .from("conversation_members")
    .insert({ conversation_id: conversation.id, user_id: myId });
  if (firstMemberError) throw firstMemberError;

  const { error: secondMemberError } = await supabase
    .from("conversation_members")
    .insert({ conversation_id: conversation.id, user_id: otherId });
  if (secondMemberError) throw secondMemberError;
...
export function subscribeToMessages(conversationId, onMessage) {
  return supabase
    .channel(`codex-chat-${conversationId}`)
    .on(
      "postgres_changes",
