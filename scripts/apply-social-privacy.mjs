...
replaceOnce(
  ...
  `import { Home, Trophy, Medal, MessageCircle, Settings, Users, Flame, Star, Clock, LogIn, Send, ArrowLeft, Heart, Camera, Plus, Trash2, User, X, Check, UserRound, LogOut, ChevronRight, Mail, Lock, Globe, Image as ImageIcon } from "lucide-react";`,
  "lucide imports"
);
...
replaceOnce(
  /\[loading,setLoading\]=useState\(true\),\[postText,setPostText\]=useState\(""\),/,
  `[loading,setLoading]=useState(true),[postText,setPostText]=useState(""),[postImage,setPostImage]=useState(null),`,
  "post image state"
);

replaceOnce(
  /const effectiveProfile=useMemo\(\(\)=>profile\|\|\{id:session\?\.user\?\.id,name:"Aventureiro",avatar_url:null,level:1,total_xp:0,quests_completed_ever:0,max_streak_ever:0,usage_seconds:0\},/,
  `const effectiveProfile=useMemo(()=>profile||{id:session?.user?.id,name:"Aventureiro",avatar_url:null,level:1,total_xp:0,quests_completed_ever:0,max_streak_ever:0,usage_seconds:0,is_private:false,equipped_title:null},`,
  "effective profile defaults"
);
...
replaceOnce(
  /const saveName=async\(\)=>\{[\s\S]*?finally\{setSaving\(false\)\}\};\n const uploadAvatar=/,
  ...
  "settings and post image handlers"
);

replaceOnce(
  /(<button[^>]*onClick=\{saveName\}[^>]*>[\s\S]*?<\/button>)/,
...
replaceOnce(
  ...
  `<div className="cv-post-photo-tools"><label className="cv-photo-button"><ImageIcon size={18}/> Foto<input type="file" accept="image/*" onChange={uploadPostImage} hidden/></label>{postImage&&<button type="button" className="cv-photo-clear" onClick={()=>setPostImage(null)}>Remover foto</button>}</div>\n$1`,
  "feed photo picker"
);

replaceOnce(
  /\{p\.text\}/,
  `{p.image_url&&<img className="cv-post-image" src={p.image_url} alt="Foto da publicação" loading="lazy"/>}{p.text}`,
  "feed photo rendering"
);

s += `\n${MARKER}\n`;
fs.writeFileSync(path, s);
console.log("Applied unique username, private profile and feed photo UI patch.");
