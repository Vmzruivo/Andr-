...
if (cardStart >= 0 && rankStart > cardStart) {
  ...
  const countBefore = s.lastIndexOf("const activeMissionCount=", cardStart);
  const candidates = [activeBefore, mpBefore, countBefore].filter(n => n >= 0);
...
const activeMissionCount=Object.values(missions||{}).filter(m=>m&&!m.cancelled&&!m.completed&&m.date===today()).length;
