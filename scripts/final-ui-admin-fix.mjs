...
const MARK='CODEX_FINAL_UI_ADMIN_FIX_V4';
if(s.includes(MARK)){console.log('final UI/admin fix already applied');process.exit(0)}

// One active mission at a time; cancelled missions can be replaced.
...
// Replace the old five-choice mission grid with one clean mission card and an expandable chooser.
const missionStart=s.indexOf('const missionCard=');
const scheduleStart=s.indexOf('const homeGoalSchedule=',missionStart);
if(missionStart<0||scheduleStart<0)throw new Error('mission markers not found');
...
// Mobile-safe layout for the board and mission chooser.
...
.cv-missions-clean .cv-quest-single{display:flex!important;flex-direction:column!important;gap:12px!important}.cv-mission-actions{display:flex!important;gap:8px!important;flex-wrap:wrap!important}.cv-mission-actions .cv-button{flex:1 1 180px!important}.cv-mission-choices{margin-top:14px;padding:12px;border:1px solid #7A4356;border-radius:14px;background:#291C2B}.cv-mission-choice-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.cv-mission-choice-grid button{min-width:0!important;padding:12px!important;border:1px solid #7A4356;border-radius:10px;background:#1E1520;color:#F5EFE6}.cv-mission-choice-grid button.active{background:#C9A455;color:#140D12}.cv-mission-choice-grid small{display:block;margin-top:4px}
@media(max-width:700px){.cv-top5{grid-template-columns:repeat(2,minmax(0,1fr))}.cv-top5-card:first-child{grid-column:1/-1}.cv-mission-choice-grid{grid-template-columns:1fr 1fr}}
`;
