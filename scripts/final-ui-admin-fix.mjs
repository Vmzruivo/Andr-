...
const MARK='CODEX_FINAL_UI_ADMIN_FIX_V2';
...
// Fix cancellation semantics: cancelled missions disappear from the active slot,
// so the user can immediately choose another mission/difficulty.
const activeOld='const activeMission=missions[difficulty]&&missions[difficulty].date===today()?missions[difficulty]:null;const mp=activeMission&&!activeMission.cancelled?missionProgress(activeMission):0;const activeMissionCount=Object.values(missions).filter(m=>m&&m.date===today()&&!m.cancelled&&!m.completed).length;';
s=s.replace(activeOld,'');
